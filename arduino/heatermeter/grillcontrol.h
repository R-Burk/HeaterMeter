...
#ifndef __GRILLCONTROL_H__
#define __GRILLCONTROL_H__
...
{
private:
  unsigned char _controlOutput;
  unsigned char _maxControlOutput;
  ...
  void commitControlOutput(void);
  void fanVoltWrite(unsigned char val);
  void adjustFeedbackVoltage(void);

public:
  GrillControl(void) : _periodCounter(0x80), _units('F') {};
  ...
  // Collection of OUTPUTFLAG_*
  void setOutputFlags(unsigned char value);
  unsigned char getOutputFlags(void) const { return _outputFlags; }
  
  /* Runtime Data */
  // Current Control output in percent, setting this will turn on manual output mode
  unsigned char getControlOutput() const { return _controlOutput; }
  void setControlOutput(int value);
  unsigned char getMaxControlOutput() const { return _maxControlOutput; }
  ...
  // true if PidOutput > 0
  boolean isOutputActive(void) const { return _controlOutput != 0; }
  // true if fan is running at maximum speed or servo wide open
  boolean isOutputMaxed(void) const { return _controlOutput >= _maxControlOutput; }
  ...
  void controlStatus(void) const;
};

#endif /* __GRILLCONTROL_H__ */
